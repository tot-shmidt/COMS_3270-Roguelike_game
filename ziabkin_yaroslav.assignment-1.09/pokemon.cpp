...
Pokemon::Pokemon(int p_level, int p_species_id, struct Database *database) {
    ...
    // 1.09: initialize HP.
    this->max_hp = actual_stats[0];
    this->current_hp = actual_stats[0];
}
...
int calculate_damage(int level, int power, int attack, int defense, bool is_stab, float type_modifier) {
    int base_damage = (((((2 * level) / 5) + 2) * power * (attack / defense)) / 50) + 2;

    // wwith modifiers
    float critical_damage = (rand() % 256 < (50 / 2)) ? 1.5 : 1.0;
    float stab = is_stab ? 1.5 : 1.0;
    float random_modifier = (rand() % 16 + 85) / 100.0f;

    int final_damage = base_damage * critical_damage * random_modifier * stab * type_modifier;
