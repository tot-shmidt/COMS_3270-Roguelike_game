 ...
 */
int move_npc(struct entity *entity, struct map *map) {
    ...
    switch (entity_type) {
        ...
        case HIKER: {
            ...
            // Found path case. Overwrite min_cost with the actual terrain cost, not value from dist_map.
                ...
                enum terrain_types next_terrain = map->grid_array[new_y][new_x];
        ...
        case PACER: {
            ...
            // Moving forward
            if (check_x > 0 && check_x < MAP_WIDTH - 1 && check_y > 0 && check_y < MAP_HEIGHT - 1) {
                enum terrain_types next_terrain = map->grid_array[check_y][check_x];
            ...
            enum directions new_direction = (current_direction + 4) % 8;
            check_y = current_y + dir_arr_y[new_direction];
            check_x = current_x + dir_arr_x[new_direction];

            if (check_x > 0 && check_x < MAP_WIDTH - 1 && check_y > 0 && check_y < MAP_HEIGHT - 1) {
                enum terrain_types prev_terrain = map->grid_array[check_y][check_x];
        ...
        case WANDERER: {
            ...
            enum terrain_types first_terrain = map->grid_array[current_y][current_x];
            enum directions current_direction = entity->npc->direction;
